...
func (t *MultiNodeTest) Run(cfg *config.Config) error {
	...
	for i := 0; i < cfg.GetNodeCount(); i++ {
		...
		for j := 0; j < batchSize; j++ {
			...
			txHash, err := transaction.QuickSend(
				client,
